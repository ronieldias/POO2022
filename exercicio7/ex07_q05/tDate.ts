...
class tDate {
    ...
    constructor(dia: number = new Date().getDate(), mes: number = new Date().getMonth() + 1, ano: number = new Date().getFullYear()) {
    ...
    public get getDia(): number {
        return this._dia;
    }
    public get getMes(): number {
        return this._mes;
    }
    ...
    public padraoBR(): string{
        return `${this.getDia}/${this.getMes}/${this.getAno}`;
    }

    public porExtenso(): string{
        let nomeMes;
        switch (this.getMes) {
    ...
    private dataValida(dia: number, mes: number, ano: number): boolean {
